...
const TrainerLogin = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await axios.post(`${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/trainer-login-direct/`, {
        email,
        password
      });
      ...
      if (response.data.require_otp) {
        ...
        const otpResponse = await axios.post(`${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/send-otp/`, {
          email,
          role: 'trainer',
          password
        });
