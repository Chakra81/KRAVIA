...
const ResumeBuilder = () => {
  ...
  const triggerAutoSave = useCallback((data, email) => {
    ...
    autoSaveTimer.current = setTimeout(async () => {
      try {
        ...
        await axios.post(`http://${window.location.hostname}:8000/api/resume/update/`, payload);
  ...
  const fetchResume = async (email) => {
    try {
      setLoading(true);
      const res = await axios.get(`http://${window.location.hostname}:8000/api/resume/?email=${email}`);
  ...
  const handleImportData = async () => {
    ...
    try {
      setLoading(true);
      const res = await axios.post(`http://${window.location.hostname}:8000/api/resume/import-data/`, { email: user.email });
      setResumeData(prev => ({ ...prev, ...res.data }));
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await axios.post(`http://${window.location.hostname}:8000/api/resume/update/`, payload);
      setResumeData(prev => ({ ...prev, ...res.data }));
  ...
  const handleDownload = async () => {
    ...
    // Notify backend
    try {
      await axios.post(`http://${window.location.hostname}:8000/api/resume/download/`, { email: user?.email });
  ...
  const handleRestoreVersion = async (versionId) => {
    try {
      setLoading(true);
      await axios.post(`http://${window.location.hostname}:8000/api/resume/restore-version/`, { email: user.email, version_id: versionId });
  ...
  const autoGenerateSummary = async () => {
    try {
      ...
      const res = await axios.post(`http://${window.location.hostname}:8000/api/resume/generate-summary/`, {
  ...
  const handleDeleteResume = async () => {
    try {
      setLoading(true);
      await axios.delete(`http://${window.location.hostname}:8000/api/resume/delete/?email=${user.email}`);
