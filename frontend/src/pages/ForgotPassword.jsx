...
const ForgotPassword = () => {
  ...
  const handleResendOtp = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await axios.post(`${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/send-otp/`, { email, role: 'admin' });
  ...
  const handleVerifyOtp = async (e) => {
    ...
    msg('', '');
    try {
      const response = await axios.post(`${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/verify-otp/`, { email, otp: otpValue });
      if (response.status === 200) {
  ...
  const handleResetPassword = async (e) => {
    ...
    msg('', '');
    try {
      const response = await axios.post(`${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/reset-password/`, { email, otp: otp.join(''), new_password: newPassword });
      if (response.status === 200) {
