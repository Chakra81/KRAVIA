...
function CollectFeeModal({ fee, onClose, onSuccess, adminEmail }) {
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/fees/collect/`, {
        method: 'POST',
...
function AddFeeModal({ onClose, onSuccess, adminEmail }) {
  ...
  useEffect(() => {
    fetch(`${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/list-students/`).then(r => r.json()).then(setStudents).catch(console.error);
    fetch(`${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/list-courses/`).then(r => r.json()).then(d => setCourses(Array.isArray(d) ? d : d.courses || [])).catch(console.error);
  }, []);

  useEffect(() => {
    if (!form.course_id) return;
    fetch(`${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/list-courses/`).then(r => r.json()).then(d => {
  ...
  const handleSubmit = async e => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/fees/`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export default function StudentFees({ onCollect }) {
  ...
  const fetchFees = useCallback(() => {
    let url = `${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/fees/?email=${user?.email}`;
  ...
  const confirmDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      const res = await fetch(`${(window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1' ? 'http://127.0.0.1:8000/api' : 'https://kravia.onrender.com/api')}/fees/${feeToDelete.id}/?email=${user?.email}`, {
        method: 'DELETE'
      });
